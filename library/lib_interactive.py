...
class LibInteractive():
    ...
    def load_gaia_cat(self, inp_cat):
        ...
        """
        if isinstance(inp_cat, Table):
            self.inp_cat = inp_cat # Save input catalogue to keep the units
    ...
    def explore_and_select(self, font_size = 16, height = 500, marker_size = 8, **kargs):
        ...
        def brush(trace, points, state):
            ...
            if inds.size:
                ...
                # self.selected         = Table.from_pandas(self.cat_subsamp)
    ...
    def export_selected(self):
        ...
        """
        if hasattr(self, 'cat_subsamp'):
            selected_0       = Table.from_pandas(self.cat_subsamp)
            for key in selected_0.keys():
                selected_0[key].unit = self.inp_cat[key].unit
            selected = Utils(color = self.color_high, label = 'selected')
            selected.read_catalogue(selected_0, verbose = False)
